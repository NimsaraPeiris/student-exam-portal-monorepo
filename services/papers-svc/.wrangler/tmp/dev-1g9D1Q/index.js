...
// .wrangler/tmp/bundle-DfZyYr/checked-fetch.js
...
app.get("/papers", async (c) => {
  ...
  const board = c.req.query("exam_board") || c.req.query("board");
  ...
  if (board) filters.push(eq(papers.exam_board, board));
  if (type) filters.push(eq(papers.type, type));
  if (year) filters.push(eq(papers.year, Number(year)));
  if (language) filters.push(eq(papers.language, language));
  const countResult = await db.select({ count: sql`count(*)` }).from(papers).where(filters.length > 0 ? and(...filters) : void 0);
...
app.post("/papers/:id/purchase", async (c) => {
  ...
  try {
    ...
    const existing = await db.select().from(purchases).where(and(eq(purchases.user_id, userId), eq(purchases.paper_id, paperId))).limit(1);
...
app.get("/papers/:id/questions", async (c) => {
  ...
  if (userId) {
    const purchase = await db.select().from(purchases).where(and(eq(purchases.user_id, userId), eq(purchases.paper_id, paperId))).limit(1);
  ...
  }).from(questions).where(eq(questions.paper_id, paperId)).orderBy(questions.order_index);
  ...
  return c.json({
    ...
    // This might be misleading if we want the actual total count
  });
});
...
// .wrangler/tmp/bundle-DfZyYr/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-DfZyYr/middleware-loader.entry.ts
