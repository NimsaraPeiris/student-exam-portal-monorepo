...
// .wrangler/tmp/bundle-XRWa5V/checked-fetch.js
...
app.get("/papers", async (c) => {
  ...
  const cacheKey = `papers_list_${subject || "all"}_${board || "all"}`;
  ...
  const result = await (filters.length > 0 ? query.where(and(...filters)) : query);
  await c.env.PAPERS_CACHE.put(cacheKey, JSON.stringify(result), { expirationTtl: 300 });
...
// .wrangler/tmp/bundle-XRWa5V/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-XRWa5V/middleware-loader.entry.ts
