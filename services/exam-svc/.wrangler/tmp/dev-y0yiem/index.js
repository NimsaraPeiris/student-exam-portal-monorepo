...
// .wrangler/tmp/bundle-0tiZ1W/checked-fetch.js
...
app.post("/exams/start", async (c) => {
  try {
    ...
    const expiresAt = new Date(startedAt.getTime() + 60 * 60 * 1e3);
...
app.get("/exams/:id/questions", async (c) => {
  ...
  try {
    ...
    const result = await db.select().from(questions).where(eq(questions.paper_id, session[0].paper_id));
    const mappedQuestions = result.map((q) => ({
      id: q.id,
      content: q.question_text,
      choices: q.options
      // matches our Choice interface { id, content }
    }));
...
app.post("/exams/:id/answer", async (c) => {
  ...
  try {
    ...
    if (/* @__PURE__ */ new Date() > new Date(session[0].expires_at)) {
      return c.json({ error: "Session has expired" }, 403);
    }
    const question = await db.select().from(questions).where(eq(questions.id, question_id)).limit(1);
    if (!question[0]) {
    ...
    return c.json({
      success: true,
      is_correct: isCorrect,
      correct_choice_id: isCorrect ? void 0 : question[0].correct_option_id
    });
  } catch (error) {
    console.error(error);
...
app.post("/exams/:id/submit", async (c) => {
  ...
  try {
    ...
    const totalQuestions = await db.select({ count: sql`count(*)` }).from(questions).where(eq(questions.paper_id, session[0].paper_id));
    const correctAnswers = await db.select({ count: sql`count(*)` }).from(session_answers).where(and(
    ...
    const updated = await db.update(exam_sessions).set({
      status: "submitted",
      submitted_at: /* @__PURE__ */ new Date(),
      score_pct: scorePct
    }).where(eq(exam_sessions.id, sessionId)).returning();
    return c.json({
...
// .wrangler/tmp/bundle-0tiZ1W/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-0tiZ1W/middleware-loader.entry.ts
